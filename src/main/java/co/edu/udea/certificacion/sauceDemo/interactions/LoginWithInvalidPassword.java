...
public class LoginWithInvalidPassword implements Interaction {
    @Override
    public <T extends Actor> void performAs(T actor) {
        ...
        WaitTime.putWaitTimeOf(800);
        actor.attemptsTo(Enter.theValue("claveInvalida").into(PASSWORD_TEXT_BOX));
        WaitTime.putWaitTimeOf(800);
        actor.attemptsTo(Scroll.to(LOGIN_BUTTON));
        WaitTime.putWaitTimeOf(800);
        actor.attemptsTo(Click.on(LOGIN_BUTTON));
    }
    public static LoginWithInvalidPassword tryLogin() {
        return Tasks.instrumented(LoginWithInvalidPassword.class);
    }
}